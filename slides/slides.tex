...
\begin{frame}
	...
	\begin{itemize}
		...
		\only<1>{
			\vspace{0.2cm}
			\item Multiplicative Extended Kalman Filter (MEKF)
			\begin{itemize}
		...
		\only<2>{
			\centerline{
				\begin{tikzpicture}
					...
					\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (5.2,-0.2) {\includegraphics[scale=0.7]{wrapping}};
				\end{tikzpicture}
			}
		}
	\end{itemize}
...
\begin{frame}
	\frametitle{Large Attitude Uncertainty}
	
	\begin{itemize}
		\item \Emph{Large Attitude Uncertainty} in Estimation Problems
		\begin{itemize}
			\item Unknown initial conditions, e.g., unknown heading direction in a building.
			\item Not properly observed degree of freedom, e.g., single reference direction that is slowly varying in the inertial frame.
			\item Sensor failure, e.g., no GPS signal in a tunnel.
		\end{itemize}
	
		\vspace{0.2cm}
		\item Goal of This Dissertation
		\begin{itemize}
			\item Deal with large attitude uncertainty using probability distribution \Emph{defined intrinsically on $\SO{3}$} for attitude.
			\begin{itemize}
				\item Matrix Fisher distribution on $\SO{3}$.
				\item Bingham distribution on $\Sph^3$ for unit quaternions.
				\item No singularities, no geometric constraints, able to model arbitrarily large attitude uncertainty.
			\end{itemize}
		
			\item Design Bayesian filters for classical estimation problems involving the attitude/pose of a rigid body.
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Matrix Fisher Distribution}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\vspace{0.2cm}
		\only<1>{
			\item Highly Concentrated in Three Degrees of Freedom \footnote{\tiny T.~Lee, ``Bayesian attitude estimation with approximate matrix {F}isher distributions on {SO}(3),'' in \emph{Conference on Decision and Control}, 2018, pp. 5319--5325.}
			\begin{theorem}
	...
	\begin{itemize}
		\only<1>{
			...
			\begin{itemize}
				...
				\begin{itemize}
					\item An algorithm to compute higher order central moments.
					\item A approximation when it is highly concentrated in two degrees of freedom.
				\end{itemize}

				\item \Emph{Cannot model the correlation between $\SO{3}$ and $\mathbb{R}^n$}.
				\item Correlation: transfer information from observed state to unobserved state.
			\end{itemize}
		...
		\only<2>{
			...
			\vspace{0.2cm}
			\item Objective: a New Model on $\SO{3}\times\mathbb{R}^n$
			\begin{itemize}
				\item Generic construction from directional statistics.
				\item \Emph{Closed form maximum likelihood estimation}.
			\end{itemize}
		}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Defined on $\SO{3}\times \mathbb{R}^n$.
			\item Extends the matrix Fisher distribution to deal with the correlation between $\SO{3}$ and $\mathbb{R}^n$.
			\item Applications: simultaneous estimation of attitude and other Euclidean quantities.
			\begin{itemize}
				\item Sensor biases.
				\item Velocity, position.
				\item Landmark locations.
			\end{itemize}
		\end{itemize}
	\end{itemize}
	...
	\vspace{0.2cm}
	\begin{itemize}
		\item Application of MFG in Attitude Estimation
		\begin{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\centerline{
			\begin{tikzpicture}
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (-3.0,0) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{prop_L_1}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{prop_L_2}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (3.0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{prop_L_3}};
		...
		\centerline{
			\begin{tikzpicture}
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (-3.0,0) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{prop_R_1}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{prop_R_2}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (3.0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{prop_R_3}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (-2.9,-1.1) {\scriptsize $t=0$s};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0.1,-1.1) {\scriptsize $t=0.5$s};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (3.0,-1.1) {\scriptsize $t=1$s};
...
\begin{frame}
	...
	\begin{itemize}
		...
		\centerline{
			\begin{tikzpicture}
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (-3.0,0) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{mea_I_1}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{mea_I_2}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (3.0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{mea_I_3}};
		...
		\centerline{
			\begin{tikzpicture}
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (-3.0,0) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{mea_B_1}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0.0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{mea_B_2}};
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (3.0,0.1) {\includegraphics[width=0.2\columnwidth, trim=100 60 100 40, clip]{mea_B_3}};
	...
	\begin{itemize}
		...
		\begin{figure}
			...
			\begin{tikzpicture}
				...
				\node at (4.5,2.8) {\includegraphics[trim=150 80 120 60, clip, scale=0.23]{est_LB_F1}};
				\node at (7.5,2.8) {\includegraphics[trim=150 80 120 60, clip, scale=0.23]{est_LB_F3}};
				
				\node at (1.5,0) {\includegraphics[trim=150 80 120 60, clip, scale=0.23]{est_LB_F5}};
				\node at (4.5,0) {\includegraphics[trim=150 80 120 60, clip, scale=0.23]{est_LB_FN_mea}};
				\node at (7.5,0) {\includegraphics[trim=150 80 120 60, clip, scale=0.23]{est_LB_FN_post}};
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{figure}
			\centering
			\begin{tikzpicture}
				\node at (1.5,2.8) {\includegraphics[trim=115 60 95 20, clip, scale=0.30]{est_LI_F1_prior}};
				\node at (4.5,2.8) {\includegraphics[trim=115 60 95 20, clip, scale=0.30]{est_LI_F1}};
				\node at (7.5,2.8) {\includegraphics[trim=115 60 95 20, clip, scale=0.30]{est_LI_F3}};
				
				\node at (1.5,0) {\includegraphics[trim=115 60 95 20, clip, scale=0.30]{est_LI_F5}};
				\node at (4.5,0) {\includegraphics[trim=115 60 95 20, clip, scale=0.30]{est_LI_FN_mea}};
				\node at (7.5,0) {\includegraphics[trim=115 60 95 20, clip, scale=0.30]{est_LI_FN_post}};
...
\begin{frame}
	...
	\begin{itemize}
		...
		\centerline{
			\begin{tikzpicture}
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0,0) {\includegraphics[scale=0.65]{attVec}};
			\end{tikzpicture}
		}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\centerline{
			\begin{tikzpicture}
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0,0) {\includegraphics[scale=0.65]{attMag}};
			\end{tikzpicture}
		}
	\end{itemize}
\end{frame}
...
\begin{frame}
	\begin{itemize}
		\item Application of MFG in Rigid Body Pose Estimation
		\begin{itemize}
		...
		\begin{itemize}
			...
			\centerline{
				\begin{beamercolorbox}[wd=10cm,sep=0.0cm,center,rounded=true,shadow=true]{numerical}
					...
					\vspace{-0.3cm}
					\begin{align*}
						f_m(R) = f_m(R)^{\lambda_1} f_m(R)^{\lambda_2} \cdots f_m(R)^{\lambda_l}, \qquad \lambda_1 + \cdots + \lambda_l = 1
					\end{align*}
					\vspace{-0.4cm}
				\end{beamercolorbox}
			}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\centerline{
			\begin{tikzpicture}
				\node[opacity=1.0,outer sep=0pt,inner sep=0pt] at (0,0) {\includegraphics[scale=0.65]{INS_error}};
			\end{tikzpicture}
		}
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		...
		\only<1>{
			\begin{tikzpicture}[inner sep=0.5cm]
				\node at (0,0) {\includegraphics[scale=0.5]{pc2_example}};
				\node at (2.5,0) {\includegraphics[scale=0.5]{pc1_example}};
				\node at (4.0,0) {\large $\xrightarrow{T\in\SE{3}}$};
				\node at (5.5,0) {\includegraphics[scale=0.5]{pc12_example}};
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Use \Emph{conditional MLE} to $p_{t|R}(t|R)$.
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Find a matrix Fisher distribution $\mathcal{M}(\tilde{F})$ through Wahba's problem.
				\item Select sigma points $\{R_i,w_i\}_{i=1}^7$ from $\mathcal{M}(\tilde{F})$.
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item The map must be built incrementally from previously captured features.
		\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\item Objective: Deal With \Emph{Large Attitude Uncertainty}
		\begin{itemize}
			\item Unknown initial conditions.
			\item Not properly observed degree of freedom for attitude.
			\item Sensor failure.
		\end{itemize}
		...
		\item Contributions
		\begin{itemize}
			\item Use probability distribution \Emph{intrinsically defined on $\SO{3}$}.
			\item Matrix Fisher distribution
			\begin{itemize}
				\item Higher order moments and highly concentrated approximations.
			\end{itemize}
		
			\item Matrix Fisher--Gaussian distribution (MFG)
			\begin{itemize}
				\item Correlation between attitude and Euclidean quantities.
			\end{itemize}
		
			\item MFG in classical estimation problems
			\begin{itemize}
				\item Attitude estimation with gyroscope and direction sensors.
				\item Loosely coupled IMU-GNSS navigation.
				\item Visual-inertial navigation/odometry.
				\item \Emph{Improved accuracy and convergence speed} when the attitude uncertainty is large.
			\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\scriptsize
		\item W.~Wang and T.~Lee, ``Matrix {F}isher-{G}aussian distribution on $\mathrm{SO}(3)\times\mathbb{R}^n$ for attitude estimation with a gyrobias,'' in \emph{American Control Conference}, 2020, pp. 4429--4434.
		\item W.~Wang and T.~Lee, ``Spectral uncertainty propagation for generalized stochastic hybrid systems with applications to a bouncing ball,'' in \emph{American Control Conference}, 2020, pp. 1803--1808.
		\item W.~Wang and T.~Lee ``Spectral {B}ayesian estimation for general stochastic hybrid systems,'' \emph{Automatica}, vol. 117, p. 108989, 2020.
		\item W.~Wang and T.~Lee, ``Higher-order central moments of matrix {F}isher distribution on $\mathrm{SO}(3)$,'' \emph{Statistics \& Probability Letters}, vol. 169, p. 108983, 2021.
		...
		2175--2191, 2021.
		\item W.~Wang and T.~Lee, ``Bingham-{G}aussian distribution on $\mathbb{S}^3\times\mathbb{R}^n$ for unscented attitude estimation,'' in \emph{International Conference on Multisensor Fusion and Integration for Intelligent Systems}, 2021, pp. 1--7.
		\item W.~Wang and T.~Lee, ``Spacecraft attitude and gyro-bias estimation with a single magnetometer on $\mathrm{SO}(3)\times\mathbb{R}^n$,'' in \emph{AIAA Scitech Forum}, 2022.
		\item W.~Wang, K.~Gamagedara, and T.~Lee, ``On the observability of attitude with single direction measurements,'' \emph{IEEE Transactions on Automatic Control}, 2022, accepted.
		\item W.~Wang and T.~Lee, ``Uncertainty propagation for general stochastic hybrid systems on compact {L}ie groups,'' \emph{SIAM Journal on Applied Dynamical Systems}, 2022, accepted.
	\end{itemize}
\end{frame}
