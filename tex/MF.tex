...
This chapter presents a new algorithms to compute the higher order moments of the matrix Fisher distribution on $\SO{3}$.
...
\begin{figure}
	\centering
	\includegraphics{figures/MF-moment-error-degenerate}
...
Nevertheless, since $\SO{3}$ has three degrees of freedom, it must be specified first the number of degrees of freedom in which the distribution is highly concentrated.
In the subsequent three subsections, the cases when the matrix Fisher distribution is highly concentrated in all three, two, and one degrees of freedom are discussed.
...
\begin{corollary}[\cite{lee2018bayesian-b}]
...
\subsection{Highly Concentrated in Two Degrees of Freedom}
...
Let $\mathbf{K} = \{\expb{\theta\hat{e}_1} \,|\, 0\leq \theta < 2\pi\}$ be a subgroup of $\SO{3}$, it is clear that $\mathbf{K}$ is isomorphic to $\SO{3} \cong \Sph^1$.
...
\end{align}
Therefore, for any $Q\in\SO{3} = \SO{3}/\mathbf{K} \times \mathbf{K} \cong \Sph^2\times \Sph^1$, it can be decomposed as $Q = T \expb{\theta\hat{e}_1}$,
...
\subsection{Highly Concentrated in One Degree of Freedom}

Finally, this subsection studies the case when $s_1+s_2\gg 0$, i.e., the matrix Fisher distribution is only highly concentrated along the third principal axis, but may have large dispersion along the first and second principal axes.
