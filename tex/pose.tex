...
Since the pseudorange from GNSS is first converted into position measurements, and then used to correct the inertial navigation estimation, this is usually referred to as \textit{loosely} coupled IMU-GNSS integration.
...
Given the IMU measurements $\{\Omega_k,a_k\}_{k=1}^\infty$, and all position, attitude, and direction measurements $\{\mathcal{Z}_k\}_{k=1}^\infty$, the problem is to find the posterior distribution of $(R_k,x_k) | \{\mathcal{Z}_m\}_{m=1}^k$, and approximate it to an MFG with parameters $(\mu_k, \Sigma_k, P_k, U_k, S_k, V_k)$.
...
One way of calculating these moments is to propagated deterministically sampled sigma points through the kinematic equations using the unscented transform of MFG, as introduced in Chapter \ref{section:attEst-propagation}.
...
In preparation for these calculations, the parameters $\mu_k$, $P_k$ and $\Sigma_k$ are written for each component in block form as
...
\begin{lemma} \label{lemma:posEst-update-factor}
	...
	\begin{align} \label{eqn:posEst-update-density-factor}
		p(R,x|\mathcal{Z}) = p_R(R) p_{x|R}(x|R),
	\end{align}
	where the marginal part $p_R(R)$ is given by
	\begin{align} \label{eqn:posEst-update-density-R}
		p_R(R) \propto \etr{\tilde{F}R^T} \expb{-\tfrac{1}{2}(H\mu_c-y)^T (\Sigma_y+H\Sigma_cH^T)^{-1} (H\mu_c-y)},
	\end{align}
	and the conditional part $f(x|R)$ is given by
	\begin{align} \label{eqn:posEst-update-density-x|R}
		p_{x|R}(x|R) \propto \expb{-\tfrac{1}{2} \big(x-(I-KH)\mu_c-Ky\big)^T \big((I-KH)\Sigma_c\big)^{-1} \big(x-(I-KH)\mu_c-Ky\big)},
	\end{align}
...
Note that the marginal part $p_R(R)$ does not depend on $x$, so according to the marginal-conditional MLE of MFG, the first step is to approximate $p_R(R)$ into the density function of a matrix Fisher distribution.
Since the $\mu_c$ is linear in $R$, $p_R(R)$ has quadratic terms of $R$ in its exponent, making the expectation $\expect{R}$ corresponding to $p_R(R)$ impractical to be calculated directly from the density function.
...
It should be noted that if the unscented transform in \cite{lee2018bayesian} is used to select sigma points in step 13, the moment $\expect{R}$ must be first matched to a matrix Fisher distribution.
...
Next, the conditional part of \eqref{eqn:posEst-update-density-factor}, i.e., the $p_{x|R}(x|R)$ in \eqref{eqn:posEst-update-density-x|R} is considered.
As $p_R(R)$ in \eqref{eqn:posEst-update-density-R} has been approximated by the density function of the matrix Fisher distribution with parameter $F^+$, the posterior density \eqref{eqn:posEst-update-density-factor} is now approximated by
\begin{align} \label{eqn:posEst-update-density-approx}
	p(R,x|\mathcal{Z}) \approx c \cdot \etr{F^+R^T} p_{x|R}(x|R),
\end{align}
...
Then to match the above density function to an MFG, the conditional MLE in Theorem \ref{thm:MFG-MLE-conditional} needs to be used, which requires the moments $\expect{x|\mathcal{Z}}$, $\expect{x(\nu_R^+)^T|\mathcal{Z}}$, and $\expect{xx^T|\mathcal{Z}}$ to be calculated.
...
The initial attitude uncertainties in the directions perpendicular to gravity, i.e., $s_1+s_2$ and $s_1+s_3$ for MFG, and the first two diagonal entries of the covariance matrix for MEKF, are determined from the accuracy of the accelerometer.
...
The measurements are affected by noises which are assumed to be Gaussian with zero mean and covariance matrices $A_i$, i.e.,
\begin{align}
...
\end{align}
The conventional method to find $(R,t)$ is to solve the maximum likelihood estimation by posting the above equation into a least square problem, and finding the minimum using Gauss-Newton or Levenberg–Marquardt algorithm.
...
\begin{theorem} \label{thm:VIO-factor}
	...
	\begin{align}
		T = \begin{bmatrix} 
			\frac{N-1}{N}  & \cdots & -\frac{1}{N}  & -\frac{1}{N}
			\\ \vdots & \ddots & \vdots & \vdots
			\\ -\frac{1}{N} & \cdots & \frac{N-1}{N} & -\frac{1}{N}
		\end{bmatrix},
	\end{align}
	...
	\end{align}
	Let $\mathbf{D}$ be written in block form as $\mathbf{D} = \big[D_1, \cdots, D_N\big]$, where $D_i\in\mathbb{R}^{3\times 3}$ for $i=1,\ldots,N$.
...
With this decomposition of the likelihood \eqref{eqn:VIO-likelihood}, the marginal part $f_R(R)$ can be first approximated by a matrix Fisher density, then the conditional part $f_{x|R}(x|R)$ can be used in Theorem \ref{thm:MFG-MLE-conditional} to recover the parameters $\mu$, $\Sigma$, and $P$ of the MFG.
...
However, the difficulty is that $\{q'_i\}$ are correlated, since $\mathbf{T}\mathbf{A}\mathbf{T}^T$ in general has nonzero off-diagonal terms.
...
\end{theorem}
\begin{proof}
	The invertibility of $TT^T$ can be verified by direction calculation.
...
\begin{align} \label{eqn:VIO-likelihood-approx}
	\expb{-\tfrac{1}{2}f(p'_1,\ldots,p'_N|R,t)} \approx c\cdot \etr{FR^T} \expb{-\tfrac{1}{2}f_{t|R}(t|R)},
\end{align}
where $c$ is a normalizing constant, and $f_{t|R}(t|R)$ is the conditional likelihood \eqref{eqn:VIO-likelihood-conditional}.
...
To use the theorem, the following moments need to be calculated: $\expect{t}$, $\expect{t\nu_R^T}$, and $\expect{tt^T}$.
...
This cannot be avoided for general cases, but in next theorem, it is shown that if all measurements have the same covariance matrix, the expressions for $\expect{t}$, $\expect{t\nu_R^T}$, and $\expect{tt^T}$ can be greatly simplified.
...
\end{theorem}
\begin{proof}
	Let $Y = \tfrac{1}{N}[1,\ldots,1]\in\mathbb{R}^{1\times N}$ so that $\mathbf{Y} = Y\otimes I_{3\times 3}$.
...
\end{align}
where the first exponential term on the right hand side is the likelihood \eqref{eqn:VIO-likelihood}, with $H = [I_{3\times 3}, 0_{3\times (n-3)}]$ selecting $t$ from $y$.
...
In Theorem \ref{thm:VIO-factor}, the likelihood $f(p'_1,\ldots,p'_N|R,t)$ has been factorized into the marginal $f_R(R)$ and the conditional $f_{t|R}(t|R)$, which can be directly applied to the posterior density.
...
\end{theorem}
\begin{proof}
	The proof is a very straightforward extension of Theorem \ref{thm:posEst-update-moments}, with $y$ in \eqref{eqn:posEst-update-density-x|R} replaced by $\mu_{t|R}$.
\end{proof}
...
The map points $\{p_i\}_{i=1}^N$ are randomly scattered as $p_i = t_t + \xi_i$, where $\xi_i\in\mathbb{R}^3$ follows a three dimensional uniform distribution on $[0,1]^3$.
...
This is not surprising because the optimization method finds the maximum likelihood estimation directly, and should have better accuracy if the algorithm converges properly.
...
Next, the pose estimation algorithms are challenged with a singular configuration when the attitude has one degree of freedom that is not properly observed.
...
There are ten map points, scattered as $p_i = [5,0,0]^T + \xi_i$, where $\xi_i\in\mathbb{R}^3$ follows a three dimensional uniform distribution on $[-1,1] \times [-a,a]^2$, and $a$ is varied in $\{1,0.1,0.01,0\}$.
...
Interestingly, when $a$ is close to zero, the translation accuracy for MFG is even better than that when $a$ is relatively large.
...
It demonstrates that MEKF has slightly better accuracy than MFG for the small initial uncertainty case, which is consistent with the results in Table \ref{tab:VIO-pose-error} for static pose estimation.
...
For example, the 3D coordinates of triangulated features from a stereo camera have larger noises when the feature is further away from the camera, so it may be beneficial to also model this difference in uncertainty after the features are added to the map.
...
\begin{table}
	...
	\begin{algorithmic}[1]
		...
		\algrule
		\State Obtain $F=$ Attitude Estimation($\{\bar{p}_i\}$, $\{p'_i\}$) in Table \ref{tab:VIO-marginal} with $A_i$ replaced by $A_i+B_i$, and $\expb{-\tfrac{1}{2}f_R(R)}$ by the first term on the right hand side of \eqref{eqn:VIO-likelihood-map-Rt-factor}.
		\State Let $F = USV^T$ be its pSVD.
		\State Calculate $\expect{\nu_R\nu_R^T}$ as in \eqref{eqn:MFG-EvRvR} using $S$.
...
These large errors in the estimated map coordinates further contribute to large pose errors in the subsequent estimation, making the filter unable to converge at all.
...
For example, if $R$ has a uniform distribution on $\SO{3}$, i.e., when $S = 0_{3\times 3}$, the shape of the density for $p_i$ is a spherical thin shell centered around $t$, which cannot be approximated by a Gaussian distribution.

As a consequence, for the third simulation case when the attitude error is initialized as $S = 0.01I_{3\times 3}$, the resulting uncertainty of the map coordinates cannot be accurately modeled by Gaussian distributions.
In order to circumvent this problem but still test the proposed algorithm in visual-inertial odometry settings, in the next simulation, only the features measured in the first second are assumed to have a map prior, and subsequent map coordinates are built incrementally from the camera measurements, modeled by Gaussian distributions with the first and second moments.
...
For the normal, and large map uncertainty cases, the MFG filter is slightly more accurate than MEKF, but most differences are statistically insignificant, indicating that the MFG and Gaussian distributions work similarly well for these cases when the attitude uncertainty is relatively small.
However, coming to the third case where the initial attitude uncertainty is very large, the MEKF again does not converge.
...
Therefore, for this odometry problem, all tested algorithms, including the MFG filters, cannot deal with wrong initial attitude as in the simulation studies.
